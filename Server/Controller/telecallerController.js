...
// 🔥 FIXED: Get Assigned Suspects for Telecaller (Direct Query Method)
const getAssignedSuspects = async (req, res) => {
  try {
    const { telecallerId } = req.params;
    console.log(`[GET_SUSPECTS] - Received request for telecallerId: ${telecallerId}`);

    // Find the telecaller to ensure they exist
    const telecaller = await Telecaller.findById(telecallerId).lean();
    if (!telecaller) {
      console.log(`[GET_SUSPECTS] - Telecaller with ID ${telecallerId} not found.`);
      return res.status(404).json({ success: false, message: "Telecaller not found" });
    }

    // Directly find all suspects assigned to this telecaller
    console.log(`[GET_SUSPECTS] - Querying suspects assigned to: ${telecallerId}`);
    ...
    if (assignedSuspects.length > 0) {
      console.log("[GET_SUSPECTS] - First suspect found:", assignedSuspects[0]);
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        assignedSuspectsCount: assignedSuspects.length,
        assignedSuspects, // Send the directly queried suspects
      },
    });
  } catch (error) {
    console.error("[GET_SUSPECTS] - Error fetching assigned suspects:", error);
    res.status(500).json({
      success: false,
      message: "Error fetching assigned suspects",
      error: error.message,
