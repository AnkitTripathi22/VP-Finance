...
exports.getAllSuspects = async (req, res) => {
  try {
    const allSuspects = await suspectModel.find({ status: "suspect" }).sort({ createdAt: -1 });
...
exports.addCallTask = async (req, res) => {
  try {
    ...
    console.log(req.body, "this is a body for testSchema");
    ...
    console.log("lkjhgfdsaqwertyuiop[");

    suspect.callTasks.push({ taskDate, taskTime, taskRemarks, taskStatus });
    await suspect.save();

    console.log(suspect, "sdfghjk");

    // Fetch the updated document from DB to confirm
    const updated = await suspectModel.findById(id);
